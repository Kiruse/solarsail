...
pub fn transform(span: Span, func: &ItemFn) -> Result<TokenStream, syn::Error> {
  if func.sig.ident != "instantiate" {
    return Err(syn::Error::new(func.span(), "`instantiate` transform can only be applied to an `instantiate` function"));
  }
  ...
  let (msg_struct, param_extractions) = match params {
    ...
    MsgParams::Msg(arg) => {
      ...
      (quote! { type InstantiateMsg = #ty; }, vec![param_extraction])
    }
  };
  ...
  let transformed_func = quote! {
    ...
    #(#func_attrs)*
    #func_vis fn instantiate(ctx: ::solarsail::ExecuteContext, #msg: InstantiateMsg) #func_return {
      ::cw2::set_contract_version(ctx.deps.storage, env!("CARGO_PKG_NAME"), env!("CARGO_PKG_VERSION"))?;
...
fn filter_attrs(attr: &Attribute) -> bool {
  !attr.path().is_ident("instantiate") && attr.path().segments.first().unwrap().ident != "authority"
}
